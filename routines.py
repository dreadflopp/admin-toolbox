...
class RoutineChip(QFrame):
    """A routine button with an edit icon overlay that appears on hover."""
    ...
    def __init__(self, display_name: str, path: Path, color: str, parent=None):
        ...
        # Edit icon as overlay on top-right of button (not in layout)
        self._edit_btn = QPushButton("\u270E")  # ✎ pencil symbol
        self._edit_btn.setParent(self._main_btn)
    ...
    def _apply_color(self, color: str) -> None:
        ...
        self._edit_btn.setStyleSheet(
            f"font-size: 12pt; padding: 0; border: none; background: rgba(0,0,0,0.2); "
            f"color: {text_color}; border-radius: 4px;"
        )

    def resizeEvent(self, event) -> None:
        super().resizeEvent(event)
        QTimer.singleShot(0, self._position_edit_btn)

    def showEvent(self, event) -> None:
        super().showEvent(event)
        QTimer.singleShot(0, self._position_edit_btn)

    def _position_edit_btn(self) -> None:
        """Position edit button as overlay on top-right of main button."""
        r = self._main_btn.rect()
        x = r.width() - 26
        y = 2
        self._edit_btn.setGeometry(x, y, 24, 24)

    def enterEvent(self, event) -> None:
        self._edit_btn.setVisible(True)
        self._position_edit_btn()
...
class RoutinesWindow(QMainWindow):
    ...
    def __init__(self, parent=None, log_fn=None):
        ...
        # --- Routines buttons scroll ---
        routines_scroll = QScrollArea()
        routines_scroll.setWidgetResizable(True)
        routines_scroll.setHorizontalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAlwaysOff)
        routines_scroll.setMaximumHeight(120)
        self._routines_widget = QWidget()
        self._routines_layout = QHBoxLayout(self._routines_widget)
        self._routines_layout.setContentsMargins(0, 0, 0, 0)
        routines_scroll.setWidget(self._routines_widget)
        layout.addWidget(routines_scroll)
        ...
        top_row.addWidget(self._btn_default)
        layout.addLayout(top_row)

        # --- Content area: stacked view/edit ---
        ...
        self._view_widget.setStyleSheet("border: 1px solid #d1d1d1; border-radius: 8px; padding: 12px;")
        ...
        self._edit_widget.setStyleSheet("border: 1px solid #d1d1d1; border-radius: 8px; padding: 12px;")
    ...
    def _refresh_ui(self) -> None:
        ...
        # Rebuild routine chips (name + pen on hover) and + button
        self._clear_buttons()
        ...
        for f in files:
            ...
            chip.pen_btn.clicked.connect(lambda checked, p=f, c=chip: self._show_routine_menu(p, c))
        ...
        m = 5
        w = 2
        painter.drawRect(11, m, w, 24 - 2 * m)
        painter.drawRect(m, 11, 24 - 2 * m, w)
        painter.end()
        plus_icon = QIcon(pix)
        ...
        self._routines_layout.addWidget(btn_plus)
    ...
    def _clear_buttons(self) -> None:
        ...
        while self._routines_layout.count() > 1:  # keep stretch
            item = self._routines_layout.takeAt(0)
            if item.widget():
                item.widget().deleteLater()
        self._chip_widgets.clear()

    def _show_routine_menu(self, path: Path, chip: RoutineChip) -> None:
        ...
        menu.exec(chip.pen_btn.mapToGlobal(chip.pen_btn.rect().bottomLeft()))
